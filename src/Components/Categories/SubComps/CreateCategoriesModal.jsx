...
const CreateCategoriesModal = () => {
  ...
  const ActiveModal = ToggleModal === 'createcategories' ? true : false
  return (
    <div className={`z-50 duration-500 h-screen bg-black/10 w-full fixed top-0 left-0 flex items-center justify-center ${ActiveModal ? 'visible opacity-100' : 'invisible opacity-0'}`}>
        <div className={`w-2/5 h-min bg-white rounded-lg animate-keep-bounce duration-200 ${ActiveModal ? 'visible translate-y-0 opacity-100 scale-100' : 'translate-y-8 scale-75 invisible opacity-0'}`}>
              
            <span className='flex items-center justify-between p-4 border-b-[1px] border-slate-50'>
                <p className='font-semibold text-[12px]'>Add Cuisine</p>
                <X onClick={()=>{dispatch(ActiveModalToggle(''))}} size={12}></X>
            </span>

            <div className='p-8'>
            <label htmlFor="CuisineName" className='space-y-4'>
              <p className='font-medium text-xs'>Cuisine Name</p>
...
//             <span className='flex items-center justify-between p-4 border-b-[1px] border-slate-50'>
//                 <p className='font-semibold text-[12px]'>Add Categories</p>
//                 <X onClick={()=>{dispatch(ActiveModalToggle(''))}} size={12}></X>
//             </span>
//         </div>
//     </div>
